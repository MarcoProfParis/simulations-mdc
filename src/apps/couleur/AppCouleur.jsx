...
const ChromaticityDiagram = forwardRef(function ChromaticityDiagram({ illuminant, onHover, userPoints = [], onAddPoint, onMovePoint, showReticle = true, onToggleReticle, showPlanckian = false, onTogglePlanckian, macadamFactor = 0, onSetMacadam, showColorFill = true, onToggleColorFill, onDblClickPoint, onToggleAllSat, allSatOn = false, annotations = [], onAnnotationsChange }, ref) {
  ...
  const handleWheel = (e) => {
    ...
    const nx0 = mx + (vx0 - mx) * factor; const ny0 = my + (vy0 - my) * factor;
    const nx1 = mx + (vx1 - mx) * factor; const ny1 = my + (vy1 - my) * factor;
        ...
        <div style={{ position: "relative" }} ref={infoRef}>
          ...
          {showInfo && (
            <div style={{
              ...
              <p style={{ margin: "0 0 8px", color: "var(--text-muted)" }}>🖱️ <strong style={{ color: "var(--text)" }}>Cliquer</strong> sur le graphique pour ajouter un point et pour le déplacer ensuite. Utiliser la fonction réticule si nécessaire <strong style={{ color: "var(--text)" }}>⊕</strong> ou la molette de la souris pour zoomer.</p>
              <p style={{ margin: "0 0 6px", color: "var(--text-muted)" }}><strong style={{ color: "var(--text)" }}>⊕ Réticule</strong> — affiche/masque le réticule de visée avec les coordonnées.</p>
      ...
      <div style={{ position: "relative", flex: 1 }}>
        <canvas
          ...
          style={{
            ...
            cursor: panStartRef.current ? "grabbing" : modeRef.current === "draw" ? "crosshair" : modeRef.current === "text" ? "text" : (dragPointRef.current ? "grabbing" : nearPoint ? "grab" : "crosshair"),
          }}
        ...
        {isZoomed && (
          <button
            onClick={resetZoom}
            style={{ position: "absolute", top: 8, left: 8, background: "var(--bg-card)", border: `0.5px solid var(--border)`, color: "var(--text)", borderRadius: 5, padding: "3px 8px", fontSize: 11, cursor: "pointer", fontWeight: 500 }}
          >↺ Zoom</button>
        )}
      </div>
    </div>
